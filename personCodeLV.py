...
def validLVPersonCode(personCode):
    if validLVPersonCodePattern(personCode):        
        ...
        if validLVPersonCodeDate(personCode[0:2]
                                 , personCode[2:4]
...
def validLVPersonCodePattern(personCode):
    if re.match("^([0-2][0-9]|(3)[0-1])(((0)[0-9])|((1)[0-2]))\\d{2}(\\-?)\\d{5}$", personCode):
        return True
    else:
        return False
...
def validLVPersonCodeControlDigit(personCode):
    ...
    subSum = sum(map(lambda multiplier: int(personCode[multiplier[0]]) * multiplier[1], enumerate(multipliers)))
